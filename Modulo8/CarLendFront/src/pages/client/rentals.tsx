...
function ClientRentalsPage() {
    ...
    const handleCancel = async (id: number) => {
        const reason = prompt('¿Por qué deseas cancelar esta renta?');
        if (!reason) return;

        try {
            await rentalService.cancel(id, { reason });
            loadRentals();
        } catch (err: any) {
    ...
    return (
        <div className={styles.container}>
            <div className={styles.header}>
                <div>
                    <h1 className={styles.title}>Mis Rentas</h1>
                    <p className={styles.subtitle}>
                        Gestiona tus rentas de vehículos
                    </p>
                </div>
            ...
            {loading ? (
                ...
                <div className={styles.empty}>
                    <p>No tienes rentas {filter !== 'all' ? getStatusLabel(filter).toLowerCase() : ''}</p>
                    <Button onClick={() => router.push('/catalog')}>
                        Explorar Vehículos
                    </Button>
                </div>
                        ...
                        <div key={rental.id} className={styles.rentalCard}>
                            ...
                            <div className={styles.rentalDetails}>
                                ...
                                <div className={styles.detailRow}>
                                    ...
                                    <span className={styles.value}>{formatCurrency(rental.basePricePerHour)}</span>
                                </div>
